...
const RandomUser: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-background text-foreground">
      ...
      {user && (
        <Card className="border-0 shadow-lg rounded-2xl overflow-hidden max-w-sm relative">
          <CardHeader className="h-32 bg-[#ef5574] relative">
            <Image
              src={user.image}
              alt={user.name}
              width={80}
          ...
          <CSSTransition
            ...
            <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-white bg-opacity-75">
              <h2 className="text-2xl font-bold text-black">✨ Thank You So Much   ✨</h2>
            </div>
          </CSSTransition>
        </Card>
      )}
